...
if [[ -n "${explicit_base}" ]]; then
  ...
  SWAGGER_BASE="${explicit_base%/}/swagger"
  echo "Using explicit swagger base: ${SWAGGER_BASE}"
else
  ...
  if curl -fsS "http://localhost:5000/swagger/index.html" >/dev/null 2>&1; then
    ...
    if [[ -n "${BASE_URL}" ]]; then
      ...
      echo "Could not determine SWAGGER_BASE (no local swagger and no BASE_URL in src/environments/environment.ts). Provide a base URL as an argument if needed." >&2
      exit 1
    fi
  fi
fi

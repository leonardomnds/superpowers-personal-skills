...
7. **Use TestingUtils para interação/injeção e modais.** Acesse serviços por `TestingUtils.inject`, dispare eventos com `TestingUtils.click`, substitua imports via `TestingUtils.replaceImport`, use `TestingUtils.mockNgbModalRef()`/`mockNgbOffcanvasRef()` para retornos de `NgbModal`/`NgbOffcanvas`, e prefira factories existentes (`generateX`) para inputs. Mantenha blocos Arrange/Act/Assert explícitos em fluxos simples e lineares.
